...
举例，同上一讲：{% math %}3 \times 4{% endmath %}矩阵
$
A=
\begin{bmatrix}
1 & 2 & 2 & 2\\
...
写出其增广矩阵（augmented matrix）{% math %}\left[\begin{array}{c|c}A & b\end{array}\right]{% endmath %}：
...
显然，有解的必要条件为{% math %}b_3-b_2-b_1=0{% endmath %}。
...
\end{bmatrix}
$
，{% math %}rank(A)=2{% endmath %}，要使{% math %}Ax=b, b \neq 0{% endmath %}有非零解，$b$必须取$A$中各列的线性组合，此时A的零空间中只有$0$向量。
